...
for ($x=0; $x<=7; $x++) {
	...
	// changed productname to product_name
	if (!empty($_POST['productname' . $x]))  {
		
		//produce item's name is not empty, so process this item
		
		
		// changed the variables in the $POST to match what was in the classes
		...
		switch ($thisProduceType) {
			...
			case "v": //veg
				...
				$produceItem = new veggie($x, $thisProductName, $thisProductPriceType, $thisProductPrice); //removed _ from thisProductName
				$typeType = "Veggie";
				break;
		}
